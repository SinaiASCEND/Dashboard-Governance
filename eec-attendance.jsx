...
function EecAttendance() {
  ...
  return (
    <div>
      ...
      {meetings.length === 0 ? (
        <PlaceholderRoster committee={committee} grouped={grouped} />
      ) : (
        <>
          {/* Summary band */}
...
// Placeholder roster used when a subcommittee has no meetings on file yet.
// Shows every tracked member with their seat info; voting members render as
// "Voting", non-voting (but true) members as "Ex-officio". Once minutes start
// being filed the full attendance matrix replaces this view.
function PlaceholderRoster({ committee, grouped }) {
  const totalVoting    = grouped.cochair.filter(m => m.seats.find(s => s.committee === committee.id)?.vote).length
                       + grouped.voting.length;
  const totalNonVoting = grouped.cochair.filter(m => !m.seats.find(s => s.committee === committee.id)?.vote).length
                       + grouped.nonvoting.length;
  const totalStudents  = grouped.student.length;
  return (
    <>
      <div style={{
        ...
        <strong style={{ color: "var(--brand-violet)" }}>Roster preview.</strong>{" "}
        Attendance records for {committee.short} are not yet filed with the Office
        of Curricular Affairs. The committee membership shown below seeds the
        attendance matrix that will populate once minutes are submitted.
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 18 }}>
        <SummaryStat label="Voting roster"     value={totalVoting} />
        <SummaryStat label="Ex-officio (non-voting)" value={totalNonVoting} />
        <SummaryStat label="Student reps"      value={totalStudents} />
        <SummaryStat label="Quorum threshold"  value={committee.quorum} />
      </div>

      <window.Card style={{ padding: 0, overflow: "hidden" }}>
        <RosterGroup label="Co-Chairs"  members={grouped.cochair}  committeeId={committee.id} tone="cochair" />
        <RosterGroup label="Voting"     members={grouped.voting}   committeeId={committee.id} tone="voting" />
        {grouped.student.length > 0 && (
          <RosterGroup label="Student reps" members={grouped.student} committeeId={committee.id} tone="student" />
        )}
        <RosterGroup label="Ex-officio (non-voting)" members={grouped.nonvoting} committeeId={committee.id} tone="exofficio" />
      </window.Card>
    </>
  );
}

function RosterGroup({ label, members, committeeId, tone }) {
  if (!members.length) return null;
  const toneColor = tone === "cochair" ? "var(--brand-violet)" :
                    tone === "exofficio" ? "var(--grey-7)" :
                    tone === "student" ? "var(--brand-magenta)" :
                    "var(--brand-cyan-deep)";
  return (
    <div>
      <div style={{
        padding: "10px 16px", background: "var(--grey-1)",
        ...
        <span>{label}</span>
        <span style={{ color: "var(--grey-7)", fontWeight: 600 }}>{members.length}</span>
      </div>
      <div>
        {members.map((m, i) => {
          ...
          return (
            <div key={m.id} style={{
              ...
              padding: "10px 16px",
              borderBottom: i === members.length - 1 ? "none" : "1px solid var(--grey-2)",
            }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink)" }}>{m.name}</div>
                <div style={{ fontSize: 11, color: "var(--grey-11)", marginTop: 2 }}>
              ...
              <span style={{
                ...
                background: tone === "exofficio" ? "var(--grey-2)" :
                            tone === "student"  ? "var(--brand-magenta-tint)" :
                            tone === "cochair"  ? "var(--brand-violet-tint)" :
                            "var(--brand-cyan-tint)",
                color: toneColor,
              }}>
                {tone === "exofficio" ? "Ex-officio" :
                 tone === "student"   ? "Student rep" :
                 tone === "cochair"   ? (seat.vote ? "Co-Chair · Voting" : "Co-Chair · Ex-officio") :
                 "Voting"}
              </span>
            </div>
          );
